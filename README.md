...
10. Scoring — `TOTAL = surface seconds + 10 × lives saved (per screen transition) + seconds left (per underground screen, banked per cycle) + 5 × cycles cleared`. The tunnel runs a visible 60s countdown that floors at 0 and never kills — the ceiling is the kill source; the countdown survives crush respawns (remaining time carries). Lives reset to 3 at each screen transition; a line-by-line count renders on the Game Over screen for runs that reached underground.
